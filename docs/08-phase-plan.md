...
| --- | --- | --- | --- |
| P0.1 | DO repo: `git init`, `.gitignore` (.env, .fernet_key, .oauth_pkce.json, _ref-*, node_modules, .venv), commit, push to private GitHub | Claude | **Done 2026-09-12** — `dev-prathap/DO`, commit `df85d52`, 1,642 files, secrets and `_ref-*` excluded |
| P0.2 | ATTEST repo: first commit, private remote | Claude | **Done 2026-09-12** — `dev-prathap/ATTEST` |
...
| P2.4 | Billing | Stripe: Team $99 / Pro $499 / usage per verified action; plan gates; invoices — **done 2026-09-15** (cloud/billing.py): plans free / team / pro / enterprise with limits (agents, retention, verified actions / month, compliance exports), `GET /v1/billing` (usage per month, remaining), 402 gates on agent slots, IETF / EU AI Act exports, retention above plan; Stripe checkout session via HTTPS (no SDK), `/stripe/webhook` with `t=…,v1=…` signature check mapping checkout / subscription / invoice events to the plan; operator `PUT /v1/billing/plan` (+ bootstrap token) for enterprise overrides. **Usage unit decided provisionally: verified actions** (doc 07). Needs a Stripe account + price ids (founder). |
| P2.5 | Adapters | Claude Agent SDK, CrewAI, **DeerFlow** (middleware), Vercel AI SDK (via HTTP API until TS SDK) — **done 2026-09-15**: `adapters/claude_agent_sdk.hooks()` (PreToolUse deny/allow/updatedInput, PostToolUse verify + record), `adapters/crewai.wrap_tools()` (subclass with attested `_run`/`_arun`), `adapters/deerflow` (outermost LangChain middleware), Vercel guide with an HTTP-API `attested()` wrapper (TS SDK is P3.2). |
| P2.6 | Distribution | Slack Marketplace listing (confirm-gate app); `verified-actions` skill on skills marketplaces; public MCP `verify` server; LangChain integrations page — **code done 2026-09-15**: `attest-mcp-server` (attest_decide / confirm / confirm_status / record / verify / ledger over stdio; readers from env tokens), `skills/verified-actions/SKILL.md`, LangChain page draft in `launch/`. Marketplace listings and the LangChain PR are the founder's (P0.4 Slack app registration first). |
| P2.7 | Launch | Public launch: Show HN, content series (3 posts), partner outreach (LangChain, Arcade, Nango) |
| P2.8 | Ops | Error tracking, uptime, backups, rate limits; SDK telemetry opt-in — **done 2026-09-15**: per-key token-bucket rate limit (`ATTEST_RATE_LIMIT`, 429 + Retry-After), optional Sentry via `SENTRY_DSN`, `deploy/backup.sh` (pg_dump, 30-day rotation), `/healthz` for uptime probes, `attest/telemetry.py` opt-in counts-only telemetry (`ATTEST_TELEMETRY=1`). |

Milestones: **M3** first paying team; **M4** public launch. Exit: ≥ 5 paying teams, 8+ deep recipes, 6 adapters, marketplaces live.

**P2 engineering status 2026-09-15:** P2.1–P2.6 and P2.8 code complete (19 recipes across 12 systems, OpenAPI driver, groups / agent overrides / retention, checkpoints + IETF / EU AI Act exports, billing gates + Stripe webhook, 7 adapters, MCP verify server, ops). Remaining are founder-side: P2.7 launch, Stripe account, Slack app + marketplace, live-account tests (P0.4). SDK 323 tests, cloud 33.
...
| ID | Component | Tasks |
| --- | --- | --- |
| P3.1 | HTTP gateway | Outbound REST proxy (`ATTEST_GATEWAY` base URL); URL-pattern recipes; works with Nango / Composio / Arcade / direct callers; single binary with MCP proxy — **done 2026-09-15**: `attest-gateway` / `attest gateway` (stdlib proxy; URL in path or `X-Attest-Upstream`; block / pending / auto; refusals answer 403 before the upstream; caller's Authorization reused for read-back — recipes for known hosts, convention or `--openapi` otherwise; `X-Attest-Level/Decision/Seq` response headers; `POST /_attest/resume/<token>`). `attest` CLI now also runs `mcp-proxy` (single binary). 11 tests incl. a socket round-trip. |
| P3.2 | TypeScript SDK | decorator/wrapper, Vercel AI SDK, Mastra, LangChain.js adapters; shared recipe registry (JSON) — **done 2026-09-15**: `packages/attest-ts` (`@attest/sdk`): canonical JSON + hashes byte-identical to Python (fixture-tested), registry from the shared `registry-data.json`, policy R0–R4 + YAML + groups/agents, JSONL hash-chained ledger, ack / custom / convention / Gmail-Slack-HubSpot recipes, console / auto / store gates + webhook, cloud client (sink with outbox, policy, confirmations), `wrap()` + `resume()`, adapters for Vercel AI SDK, LangChain.js, Mastra. 24 vitest tests; CI job. Recipe catalogue beyond the three is Python-only. |
| P3.3 | Cloud auth (optional) | Nango self-hosted for read-only verification credentials when the agent holds no token (API-only / no-code users) — **done 2026-09-15**: org settings `nango_url / nango_secret / nango_connections`, `POST /v1/verify` (server-side ladder with Nango tokens fetched per call, never stored; `record: true` appends a row), SDK `Attest.cloud(cloud_verify=True)` adds `CloudVerifyDriver` as the last-resort driver. |
| P3.4 | Recipe automation | LLM-assisted recipe generation (DeerFlow internal tool): read docs / MCP tool lists ⇒ propose pairs ⇒ human review ⇒ registry; **open recipe registry** (community, versioned) — **done 2026-09-15**: declarative JSON recipes (`registry/recipes/`, `$ATTEST_RECIPES_DIR`, `~/.attest/recipes`) with templated read URLs, field lists and checks; `attest recipes propose` from an OpenAPI spec (fields = request body ∩ read response) or an MCP tools list (create_X ⇒ get_X), optional `--llm` refinement via the Anthropic API (`claude-fable-5-1`) that only suggests compare fields; `install` after review; `list`. Proposals are never auto-installed. DeerFlow as the generation tool is replaced by the CLI + model call. |
| P3.5 | Hardening | signed checkpoints + external anchoring option, SSO (SAML/OIDC), on-prem ledger image, SOC 2 readiness checklist — **done 2026-09-15**: `attest anchor` with File / Git / HTTP anchors + `--verify`; optional Ed25519 signing (`attest[signing]`); OIDC authorization-code login on the cloud with signed session cookies mapped to orgs by email domain (`sso_domains`, `sso_admins`, `sso_default_role`) — SAML via the IdP's OIDC bridge; `deploy/on-prem/` compose (SQLite, no external services); `docs/site/soc2.md` checklist. |
| P3.6 | Product | Teams/email confirm channels, agent-activity digests, anomaly flags (unusual verb/target patterns) — **done 2026-09-15**: `TeamsNotifier` (Adaptive Card via incoming webhook), `EmailNotifier` (SMTP to the approver group), both with signed one-click Approve / Reject links served by `attest serve` (`/decide/{id}/{status}?exp&sig`, `ATTEST_LINK_SECRET`); `attest digest` / `GET /v1/digest` with Slack posting; anomaly flags new_action, volume_spike, unverified_spike, off_hours, external_burst against a 7-day baseline. |

Exit: any language covered; recipe long tail growing automatically; enterprise-readiness path visible.

**P3 engineering status 2026-09-15:** P3.1–P3.6 code complete. SDK 349 tests, cloud 37, TypeScript 24, all in CI. Founder-side: publish `@attest/sdk` to npm and `attest` to PyPI (name decision), a real OIDC provider run-through, first anchor target, live-account recipes.
